...
# 2. Récupération de la clé depuis les Secrets de Streamlit
...
# 4. Barre latérale pour les documents
...
# 6. Traitement des messages envoyés par l'utilisateur
if user_query := st.chat_input("Posez votre question..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Votre conseiller répond..."):
            try:
                # Requête vers l'API Google Gemini
                ...
                if 'candidates' in response_data and len(response_data['candidates']) > 0:
                    ...
                    error_msg = response_data.get('error', {}).get('message', 'Clé invalide ou incorrecte')
